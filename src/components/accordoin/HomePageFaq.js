...
import { homePageFaQData } from "../../constants";
...
const HomePageFaq = () => {
  ...
  return (
    <section className="h-[100vh] md:h-[100vh] flex flex-col justify-center items-center py-10 mt-20 p-5  ">
      <h1 className="text-6xl max-sm:text-3xl sm:text-4xl md:text-5xl xl:text-6xl max-xl:text-7xl max-sm:mx-3 font-abelPro font-bold text-[#f80400] mb-0">
        Quick Answers to{" "}
      ...
      <div className="w-full grid place-items-center overflow-auto scrollbar-hide ">
        <div className="h-[100vh] flex flex-col mt-10">
          <div className="px-4 md:px-5 max-w-[800px] ">
            {homePageFaQData.map((data, index) => {
              return (
                <Accotdoin
                  key={index}
                  open={index === open}
