  ...
  return (
    <div className="flex flex-col justify-center items-center p-5">
      ...
      <p className="text-base font-abelPro p-2 mt-3 text-center">
        Nodex Systems unites IoT experts that have launched IoT applications for
      ...
      <div className="w-[90vw] grid max-xl:grid-cols-3 xl:grid-cols-3 lg:grid-cols-3 md:grid-cols-1 sm:grid-cols-1 max-sm:grid-cols-1 gap-10 p-10 mx-5">
        {sections.map((section, index) => (
          <div
            key={index}
            className={`border-[2px] border-designColor rounded-xl shadow-2xl p-4 cursor-pointer transition-all ${
            ...
            {expandedSection === index && (
              <div className="w-full h-[20px] bg-designColor mt-2"></div>
            )}
          </div>
        ))}
      </div>
